...
async function findBudgetConflict(
  ...
  try {
    return await pb.collection('budgets').getFirstListItem(
      `user = "${escapeFilterValue(userId)}" && month = "${escapeFilterValue(month)}" && category = "${escapeFilterValue(category)}" && id != "${escapeFilterValue(excludeId)}"`
    )
  } catch (error: unknown) {
...
export async function PUT(
  ...
  try {
    ...
    const existing = await pb.collection('budgets').getOne(id)
    if (existing.user !== userId) {
      return NextResponse.json({ error: 'Not found' }, { status: 404 })
    }

    const body = await request.json()
    const parsed = parseBudgetPayload(body, { partial: true })
    if (!parsed.data) {
      return NextResponse.json({ error: parsed.error || 'Invalid budget payload' }, { status: 400 })
    }

    const nextMonth = String(parsed.data.month ?? existing.month ?? '')
    const nextCategory = String(parsed.data.category ?? existing.category ?? '')
    const duplicate = await findBudgetConflict(pb, userId, nextMonth, nextCategory, id)
    if (duplicate) {
    ...
    const record = await pb.collection('budgets').update(id, parsed.data)
